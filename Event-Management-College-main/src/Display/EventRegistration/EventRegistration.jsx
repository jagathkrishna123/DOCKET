...
const EventRegistration = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        const userRegsRes = await axios.get(`${API_BASE_URL}/registrations/user/${loggedInUser._id}`);
        const userRegs = userRegsRes.data.data || [];
        const isAlreadyRegistered = userRegs.some(reg => {
          const regEventId = reg.eventId?._id || reg.eventId;
          return regEventId === foundEvent._id;
        });
        ...
        if (foundEvent.participationType === "team") {
          setTeamData({
            ...
            members: Array.from(
              ...
              (_, i) => i === 0
                ? { name: loggedInUser.name, regNo: loggedInUser.registerNumber, dept: loggedInUser.department }
                : { name: "", regNo: "", dept: "" }
            ),
          });
        }
  ...
  const checkLimits = (event, user, allRegs) => {
    ...
    const eventRegs = allRegs.filter(r => {
      ...
      return regEventId === event._id && r.status !== "rejected";
    });

    if (event.participationType === "individual") {
      // Overall Limit
      if (event.overallIndividualLimit > 0 && eventRegs.length >= Number(event.overallIndividualLimit)) {
        setIsLimitReached(true);
      ...
      // Department Limit
      const deptRegs = eventRegs.filter(r => r.registeredUserDept === user.department);
      if (event.departmentIndividualLimit > 0 && deptRegs.length >= Number(event.departmentIndividualLimit)) {
        setIsLimitReached(true);
        setLimitMessage(`Your department (${user.department}) has reached its maximum slot limit of ${event.departmentIndividualLimit}.`);
      }
    } else {
      // Team Limits
      const deptTeams = eventRegs.filter(r => r.teamData?.department === user.department);
      if (event.teamsPerDepartment > 0 && deptTeams.length >= Number(event.teamsPerDepartment)) {
        setIsLimitReached(true);
        setLimitMessage(`Maximum teams allowed from your department (${user.department}) has been reached (${event.teamsPerDepartment} teams).`);
      }
    }
  };
                    ...
                    <div key={index} className={`grid grid-cols-1 md:grid-cols-2 gap-4 p-6 rounded-3xl border transition-all ${index === 0 ? 'bg-emerald-600/10 border-emerald-500/30' : 'bg-white/[0.02] border-white/5'}`}>
                              ...
                              axios.get(`${API_BASE_URL}/users/lookup/${newRegNo}`)
                                .then(res => {
                                  if (res.data && res.data.name) {
                                    // Optional: Restriction check for department if the schema implies it
                                    if (res.data.department !== user.department) {
                                      toast.warning(`${res.data.name} is from ${res.data.department}. Ensure this complies with event rules.`);
                                    }
                                    handleMemberChange(index, "name", res.data.name);
                                    handleMemberChange(index, "dept", res.data.department);
                                  } else {
                                    handleMemberChange(index, "name", "");
                                    handleMemberChange(index, "dept", "");
                                  }
                                }).catch(err => {
                                  console.error("Lookup error:", err);
                                  handleMemberChange(index, "name", "NOT_FOUND");
                                  handleMemberChange(index, "dept", "");
                                });
                            } else if (index !== 0) {
                              handleMemberChange(index, "name", "");
                      ...
                      <div className="space-y-3">
                        ...
                        <input
                          ...
                          value={member.name === "NOT_FOUND" ? "" : member.name}
                          onChange={(e) => {
