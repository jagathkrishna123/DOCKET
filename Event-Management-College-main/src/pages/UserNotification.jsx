...
const UserNotification = () => {
    ...
    const fetchNotifications = async () => {
        try {
            ...
            // Access .data.data for wrapped response
            const notificationsArray = response.data?.data || [];
            // Filter for 'all' (Programs) or 'everyone' (Universal Broadcasts)
            const filtered = notificationsArray.filter(n => n.recipientType === "all" || n.recipientType === "everyone");
                                    ...
                                    <div className="flex flex-col md:flex-row gap-8">
                                        ...
                                        {notification.images && (
                                            <div className="w-full md:w-48 h-48 md:h-auto rounded-3xl overflow-hidden border border-white/10 flex-shrink-0">
                                                <img
                                                    src={`${API_BASE_URL}/uploads/${notification.images}`}
                                                    alt="Program"
