...
const AddEvent = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (isEditMode) {
        // Fallback to location state if available, but fetch to be sure
        const eventToEdit = location.state?.eventData;
        if (eventToEdit) {
          setEventData({
            ...eventToEdit,
            date: eventToEdit.date ? eventToEdit.date.split('T')[0] : "",
            poster: eventToEdit.poster || null,
            priceImage: eventToEdit.priceImage || null,
            sponsorImages: eventToEdit.sponsorImages || [],
            incharge: eventToEdit.incharge || "",
          });
        } else {
          const eventsRes = await axios.get(`${API_BASE_URL}/events`);
          const fetchedEvent = eventsRes.data.find(e => e._id === id || e.id == id);
          if (fetchedEvent) {
            setEventData({
              ...fetchedEvent,
              date: fetchedEvent.date ? fetchedEvent.date.split('T')[0] : "",
              poster: fetchedEvent.poster || null,
              priceImage: fetchedEvent.priceImage || null,
              sponsorImages: fetchedEvent.sponsorImages || [],
              incharge: fetchedEvent.incharge || "",
            });
          }
        }
      }
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
              ...
              <div className="space-y-4">
                ...
                <div className="relative group aspect-square rounded-[2rem] bg-white/[0.02] border-2 border-dashed border-white/10 hover:border-blue-500/30 transition-all flex flex-col items-center justify-center p-6 text-center cursor-pointer overflow-hidden">
                  {eventData.poster ? (
                    <img src={typeof eventData.poster === 'string' ? `${API_BASE_URL}/uploads/${eventData.poster}` : URL.createObjectURL(eventData.poster)} className="absolute inset-0 w-full h-full object-cover" alt="Poster" />
                  ) : (
              ...
              <div className="space-y-4">
                ...
                <div className="relative group aspect-square rounded-[2rem] bg-white/[0.02] border-2 border-dashed border-white/10 hover:border-amber-500/30 transition-all flex flex-col items-center justify-center p-6 text-center cursor-pointer overflow-hidden">
                  {eventData.priceImage ? (
                    <img src={typeof eventData.priceImage === 'string' ? `${API_BASE_URL}/uploads/${eventData.priceImage}` : URL.createObjectURL(eventData.priceImage)} className="absolute inset-0 w-full h-full object-cover" alt="Price" />
                  ) : (
                    <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 h-full">
                    {eventData.sponsorImages.map((img, i) => (
                      <div key={i} className="relative rounded-xl overflow-hidden border border-white/10 aspect-square">
                        <img src={typeof img === 'string' ? `${API_BASE_URL}/uploads/${img}` : URL.createObjectURL(img)} className="w-full h-full object-cover" />
                        <button
                          type="button"
