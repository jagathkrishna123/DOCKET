...
const Registrations = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      console.log("Registrations:", regsRes.data);
      console.log("Users:", studentsRes.data);
  ...
  const getEventStudents = (eventId) => {
    const eventRegs = registrations.filter(reg => {
    ...
    eventRegs.forEach(reg => {
      if (reg.teamData && reg.teamData.members && reg.teamData.members.length > 0) {
        ...
        const student = users.find(u => String(u._id) === String(reg.userId?._id || reg.userId));
        if (student) {
          allStudents.push({
            ...student,
            regId: reg._id,
  ...
  const selectedEventStudents = selectedEvent ? getEventStudents(selectedEvent._id) : [];
      ...
      {selectedEvent ? (
              ...
              <table className="w-full text-left text-gray-300">
                ...
                <tbody className="divide-y divide-white/5">
                  {selectedEventStudents.map((student) => (
                    <tr key={student._id || student.id} className="hover:bg-white/[0.02] transition-colors">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {filteredEvents.map((event) => {
            const count = getEventStudents(event._id).length;
            return (
              <div
                key={event._id}
